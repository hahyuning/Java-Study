...
## static
- static으로 선언한 변수는 객체의 변수가 되는 것이 아니라 클래스의 변수가 되기 때문에 클래스 변수라고 한다.
- 다른 JVM에서는 static이라고 선언해도 다른 주소나 다른 값을 참조하지만, 하나의 JVM이나 WAS 인스턴스에서는 같은 주소에 존재하는 값을 참조한다.
- GC의 대상이 되지 않는다.
- 웹 환경에서 static을 잘못 사용하면 여러 쓰레드에서 하나의 변수에 접근할 수도 있기 때문에 데이터가 꼬이는 문제가 발생할 수 있다.

#### static 활용법
- 만약 자주 변경되지 않고, 경우의 수가 단순한 쿼리 문장이 있다면 final static이나 static으로 선언하여 사용한다.
  - 자주 사용되는 로그인 관련 쿼리들이나 간단한 목록 조회 쿼리를 final static으로 선언하면 적어도 1바이트 이상의 객체가 GC 대상에 포함되지 않는다.
  - 간단한 코드성 데이터들은 static으로 선언해 놓으면 편리하다.
  - 템플릿 성격의 객체를 static으로 선언하는 것도 성능 향상에 도움이 된다. (Velocity)
- 클래스의 객체를 생성할 때마다 설정 파일을 로딩하면 엄청난 성능 저하가 발생하므로, 설정 파일 정보는 static으로 데이터를 읽어서 관래한다.
- 건수가 많지 않되 고죄 빈도가 높은 코드성 데이터는 DB에서 한 번만 읽어서 관리하는 것이 성능 측면에서 좋다.
